...
--             m LA.EigenData
-- jacobiP arr = 
--   let   (Z:.dim :. _) = R.extent arr
--         mtx = fromLists . LA.vector2Matrix dim . R.toList  $ arr
--         (eigenvals,eigenvecs) = eigSH mtx
--         repaEigenVecs =  R.fromListUnboxed (Z:.dim :. dim) . NLA.toList . flatten $ eigenvecs
--         repaEigenVals =  R.fromListUnboxed (Z:.dim) . NLA.toList $ eigenvals
--   in return $ EigenData (toUnboxed repaEigenVals) repaEigenVecs
