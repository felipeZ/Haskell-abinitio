...
import Science.QuantumChemistry.Integrals.IntegralsEvaluation (mtxOverlap)
...
doSomeStuff optsR@Options { optMode = mode } = 
    case mode of
...
check_data_dir dir optsR = do
    ...
    if test then return $ optsR { optDataDir =  Just dir } 
            else left $ "Data directory " ++ dir ++ " does not exist" 
