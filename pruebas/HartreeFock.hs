...
calcIntegrals coords basis nelec = M.fromAscList $ DL.zip miniKeys integrals
  where integrals = evalIntbykey coords basis miniKeys
        dim = nelec - 1
        cartProd = [[i,j,k,l] |  i <- [0..dim], j <- [0..dim], k <- [0..dim], l <- [0..dim]]
        miniKeys = filter condition cartProd
...
map2Array :: M.Map [Int] Double
             ...
             -> Nelec
             -> Array D DIM1 Double
map2Array mapIntegrals sortKeys (i,l) ne =
  R.fromFunction (Z:.ne)
     (\(Z:.indx) ->
